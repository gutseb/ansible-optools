#!/bin/bash
#
# Glance API monitoring script for Sensu
#
...
if ! KEY=$(glance image-list 2>/dev/null)
then
    echo "Unable to list images"
    exit $STATE_CRITICAL
fi

count=$(($(glance image-list 2>/dev/null | wc -l)-4))
responsetime=$((time -p glance image-list) 2>&1 | grep real | awk '{print $2}')

echo "Glance API is working with $count images. (response time = $responsetime)"
