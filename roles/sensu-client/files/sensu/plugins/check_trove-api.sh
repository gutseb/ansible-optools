#!/bin/bash
#
# Trove API monitoring script for Sensu
#
...
usage ()
{
    ...
    echo " -E <Endpoint>    URL for Trove endpoint. Optional. If blank, use service catalog"
}
...
if ! which trove >/dev/null 2>&1
then
    echo "python-troveclient is not installed."
    exit $STATE_UNKNOWN
fi


if ! KEY=$(trove list 2>/dev/null)
then
    echo "Unable to list instances"
    exit $STATE_CRITICAL
fi

count=$(($(trove list 2>/dev/null | wc -l)-4))
responsetime=$((time -p trove list) 2>&1 | grep real | awk '{print $2}')

echo "Trove API is working with $count instances. (response time = $responsetime)"
