...
def print_metrics(replica_status):
    status_ok()
    metric('galera_wsrep_replicated_bytes', 'int64',
           replica_status['wsrep_replicated_bytes'], 'bytes')
    metric('galera_wsrep_received_bytes', 'int64',
           replica_status['wsrep_received_bytes'], 'bytes')
    metric('galera_wsrep_commit_window_size', 'double',
           replica_status['wsrep_commit_window'], 'sequence_delta')
    metric('galera_wsrep_cluster_size', 'int64',
           replica_status['wsrep_cluster_size'], 'nodes')
    metric('galera_queries_per_second', 'int64',
           replica_status['Queries'], 'qps')
    metric('galera_wsrep_cluster_state_uuid', 'string',
           replica_status['wsrep_cluster_state_uuid'])
    metric('galera_wsrep_cluster_status', 'string',
           replica_status['wsrep_cluster_status'])
    metric('galera_wsrep_local_state_uuid', 'string',
           replica_status['wsrep_local_state_uuid'])
    metric('galera_wsrep_local_state_comment', 'string',
