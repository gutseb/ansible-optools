...
OPENRC = '/etc/sensu/keystonerc_sensu'
TOKEN_FILE = '/etc/sensu/.auth_ref.json'
...
try:
    ...
    from ceilometerclient import exc as ce_exc
except ImportError:
...
def metric(name, metric_type, value, unit=None):
    ...
    metric_line = 'sensu.%s.%s %s %s' % (host, name, value, timestamp)
    #if unit is not None:
