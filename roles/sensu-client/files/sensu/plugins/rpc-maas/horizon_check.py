...
def check(args):
    ...
    try:
        ...
        try:
            ...
            l = s.post(
                ('%s:%s/dashboard/auth/login/') % (HORIZON_URL, HORIZON_PORT),
                data=payload,
                verify=False)
        except (exc.ConnectionError,
                exc.HTTPError,
    ...
    if is_up:
        metric('horizon_splash_status_code', 'uint32', splash_status_code, 'http_code')
        metric('horizon_splash_milliseconds', 'double', splash_milliseconds, 'ms')
        metric('horizon_login_status_code', 'uint32', login_status_code, 'http_code')
        metric('horizon_login_milliseconds', 'double', login_milliseconds, 'ms')
