#!/bin/bash
#
# Cinder API monitoring script for Sensu
#
...
if ! KEY=$(cinder list 2>/dev/null)
then
    echo "Unable to list volumes"
    exit $STATE_CRITICAL
fi

count=$(($(cinder list 2>/dev/null | wc -l)-4))
responsetime=$((time -p cinder list) 2>&1 | grep real | awk '{print $2}')

echo "Cinder API is working with $count volumes. (response time = $responsetime)"
